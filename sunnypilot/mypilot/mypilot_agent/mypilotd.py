...
def main() -> None:
    ...
    # Live map GPS pin (single low-rate gpsLocation reader; see backends/real.py SAFETY note). Opt-in
    # and OFF unless set. Source it from the PERSISTENT config.json (env vars in launch_env.sh do NOT
    # survive a boot on a prebuilt branch — that file is git-tracked and reset on update). The backend
    # reads MYPILOT_LIVE_GPS from the env, so promote the config value into the env here.
    if cfg_file.get("live_gps") in (True, 1, "1", "true", "True"):
        os.environ["MYPILOT_LIVE_GPS"] = "1"
        print("[mypilotd] live GPS map pin ENABLED (config.json live_gps)")
