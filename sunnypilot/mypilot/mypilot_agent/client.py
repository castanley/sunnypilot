...
class StackClient:
    ...
    async def register_complete(self, pairing_id: str, identity: Identity) -> dict:
        ...
        async with self.http.post(
            ...
            # Transient server states (429 rate-limit, 5xx) must NOT crash the agent. Before this, a
            # 429 propagated out of pair() -> process exit -> restart_if_crash respawn -> immediate
            # retry -> another 429: a self-reinforcing crash-loop that hammered the Stack (observed
            # ~20k restarts on an unpaired device). Surface it as a soft "retry" so pair() backs off.
            if resp.status == 429 or resp.status >= 500:
                return {"status": "retry", "http_status": resp.status}
            resp.raise_for_status()
            return await resp.json()
