...
class IngestClient:
    ...
    async def upload_route(self, route: dict, *, allow_archive: bool = True) -> list[str]:
        ...
        ``allow_archive`` (default True): when False (drive_upload="full_wifi" on cellular/metered), the
        heavy full-res archive tier is HELD — the preview tier still uploads (drive stays viewable), and
        the archive files are simply left un-marked so a later cycle (once on WiFi) re-uploads them via
        the existing deferred-retry. The preview tier is NEVER gated by this flag."""
        decls = [
        ...
        # Hold the heavy archive when not allowed (full_wifi on cellular): leave those files un-marked
        # so the next cycle retries them on WiFi. The preview tier below is unaffected.
        archive = [f for f in route["files"] if f["kind"] not in self._PLAYABLE_KINDS] if allow_archive else []
